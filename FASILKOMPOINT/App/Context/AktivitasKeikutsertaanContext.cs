...
namespace FASILKOMPOINT.App.Context
{
    internal class AktivitasKeikutsertaanContext : DatabaseWrapper
    {
        ...
        {
            string query = $"DO $$ DECLARE new_id_aktivitas INTEGER; BEGIN INSERT INTO {table} (id_aktivitas, nama_kegiatan, tanggal_dimulai, tanggal_berakhir, file_bukti, jenis_kepesertaan, level_tingkat, poin_id_poin, mahasiswa_username, is_acc, kategori_id_kategori) VALUES (COALESCE((SELECT MAX(id_aktivitas) FROM {table}), 0)+1, @nama_kegiatan, @tanggal_dimulai, @tanggal_berakhir, @file_bukti, @jenis_kepesertaan, @level_tingkat, (SELECT id_poin FROM poin WHERE butir_id_butir = (SELECT id_butir FROM butir WHERE nama_butir = @level_tingkat) AND subbutir_id_subbutir = (SELECT id_subbutir FROM subbutir WHERE nama_butir = @jenis_kepesertaan) AND kategori_id_kategori = @kategori), @mahasiswa_username, @is_acc, @kategori) RETURNING id_aktivitas INTO new_id_aktivitas; INSERT INTO komentar (id_komentar, aktivitas_id_aktivitas, komentar, tim_skpi_username) VALUES (COALESCE((SELECT MAX(id_komentar) FROM komentar), 0) + 10, new_id_aktivitas, 'Belum ada komentar', '198706192014041001'); END $$;\r\n";
            NpgsqlParameter[] parameters =
            {
