...
namespace FASILKOMPOINT.View.TU
{
    public partial class BerandaTU : Form
    {
        ...
        {
            // Memastikan bahwa tombol yang diklik berada di kolom dengan nama "PDFButton"
            if (e.ColumnIndex == dataGridView1.Columns["PDFButton"].Index && e.RowIndex >= 0)
            {
                // Mendapatkan nilai NIM atau identifikasi unik dari baris yang diklik
                string nimMahasiswa = dataGridView1.Rows[e.RowIndex].Cells["nim"].Value.ToString();

                // Panggil metode untuk membuat PDF menggunakan QuestPDF
                GeneratePDF();
            }
        }
